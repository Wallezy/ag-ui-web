...
export type BackendDiagnostics = {
  ...
  expectedFrontendRevision: string
...
export function parseBackendDiagnostics(
  ...
  const config = record(health.oaV2EffectiveConfig)
  if (!config) return null
  const calibration = record(config.calibration)

  return {
    status: boundedText(health.status) ?? 'UNKNOWN',
    rolloutStage: boundedText(config.rolloutStage) ?? 'unknown',
    backendRevision: revision(config.backendRevision),
    expectedFrontendRevision: revision(config.frontendRevision),
...
export function displayRevision(value: string) {
  return /^[a-f0-9]{40}$/i.test(value) ? value.slice(0, 12) : value
}

function revision(value: unknown) {
  const parsed = boundedText(value, 200) ?? 'unavailable'
  return /^[A-Za-z0-9_.:-]+$/.test(parsed) ? parsed : 'unavailable'
}
